...
函数作用：要开辟n个长度为size的连续空间。函数的返回值是所分配区域的第一个字节的地址，开辟空间失败返回NULL
形参n的类型定义为无符号类型，表示长度为size的空间的个数
形参 size 的类型定义为无符号类型，表示开辟空间的长度
calloc函数开辟的空间比较大，可以为一维数组开辟动态存储空间，其中n维元素个数，每个元素长度为size
...
该函数的作用：对于那些使用malloc、calloc函数开辟的动态控件，如果想要改变空间大小，可以使用该函数重新分配。
参数p 是void类型的指针变量，要指向一个动态存储区
形参 size 的类型定义为无符号类型，示开辟空间的长度
